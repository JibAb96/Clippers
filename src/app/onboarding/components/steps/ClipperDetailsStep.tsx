...
const ClipperDetailsStep: React.FC<ClipperDetailsStepProps> = ({
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (formData.followerCount < 0 || formData.followerCount > 500000000) {
      newErrors.followerCount =
        "Follower count must be between 0 and 500,000,000";
    }

    if (formData.pricePerPost < 0 || formData.pricePerPost > 500000000) {
      newErrors.pricePerPost =
        "Price per post must be between 0 and 500,000,000";
    }
  ...
  const handlePriceChange = (value: string) => {
    ...
    if (errors.pricePerPost)
      setErrors((prev) => ({ ...prev, pricePerPost: undefined }));
  };
