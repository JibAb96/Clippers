...
const Filter = () => {
  ...
  const selectedFilters = useAppSelector((state) => state.selectCategory);
  ...
  return (
    <div className="flex items-center w-full relative border-t border-tertiary">
      {/* Sort button */}
      ...
      <button
        className={`z-10 bg-white bg-opacity-90 hover:text-secondary text-black font-bold py-2 px-2 disabled:opacity-0 ${
          disableButtons ? "opacity-0 cursor-not-allowed" : ""
        }`}
        onClick={scrollLeft}
