...
const Form = () => {
  ...
  useEffect(() => {
    console.log(
      "[Login Form useEffect] Running effect - User:",
      user,
      "Loading:",
      loading,
      "Error:",
    ...
    if (user && !loading && !error && loggedInUserType) {
      console.log(
        "[Login Form useEffect] Conditions met for navigation. User data:",
        user,
        "Role:",
        loggedInUserType
      );
      if (loggedInUserType === "creator") {
        router.push("/dashboard/creator"); // Or your actual creator dashboard path
      } else if (loggedInUserType === "clipper") {
        router.push("/dashboard/clipper"); // Or your actual clipper dashboard path
      } else {
        router.push("/"); // Fallback, though userType should be set
      }
    }
