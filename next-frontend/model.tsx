...
import clippers from "./src/clippers";
...
export type Clipper = {
  ...
  thumbnail: StaticImageData; // URL to the thumbnail image for the clipper
  images: StaticImageData[]; // Array of image URLs representing the clipper
...
export type Clip = {
  ...
  thumbnail: StaticImageData; // Thumbnail image for the clip
};
  ...
  switch (actions.type) {
    case ACTIONS.FILTER_CLIPPERS:
      ...
      if (actions.payload.selectedFilters.length > 0) {
        const tempClippers = actions.payload.selectedFilters.map((selectedCategory) => {
          // Filter clippers by matching niche or platform with selected category
          const temp = clippers.filter(
            (clipper) =>
...
export type Profile = {
  ...
  engagementImages?: StaticImageData[]; // Only for Clippers - Up to 4 images 
